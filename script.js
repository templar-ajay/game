...
highScoreDisplay.innerHTML = `Highest Score: ${
    ...
    for (let i = 0; i < yl; i++) {
      ...
      for (let k = 0; k < 155; k++) {
        pattern += `<td>${
          ...
          i == 0 ||
          k == 154 ||
          renderObj(allObjects, i, k) ||
          renderRaven(corvus, i, k)
            ? "*"
            : "&nbsp;&nbsp;"
        }</td>`;
      }
...
render(frameMove, collide, countPoints);
...
function collide() {
  for (let i = 0; i < corvus.length; i++) {
    for (let k = 0; k < allObjects.length; k++) {
      if (
        corvus[i][0] == allObjects[k][0] &&
        corvus[i][1] == allObjects[k][1]
      ) {
        gameOver();
      }
    }
